@extends('layouts.adminmaster')
								...
								<div class="card ">
									<div class="card-header border-0 d-md-max-block">
										...
										<div class="card-options d-md-max-block">
											...
											<a href="javascript:void(0)" class="btn btn-danger me-3  mb-md-max-2 mw-10" id="projectsassign">
											{{trans('langconvert.admindashboard.assignprojects')}}</a>

											<a href="javascript:void(0)" class="btn btn-danger me-3  mb-md-max-2 mw-10" id="projectsassigns">
											<!-- {{trans('langconvert.admindashboard.assignprojects')}} -->
											Assign Sub Project
										</a>
											...
											<table class="table table-vcenter text-nowrap table-bordered table-striped ticketdeleterow w-100" id="support-articlelists">
												<thead>
													<tr>
														...
														<th >{{trans('langconvert.adminmenu.sub_projects')}}</th>
    ...
    @include('admin.projects.model')
            ...
            <!-- Add Sub Projectz Assign-->
            <div class="modal fade subproject_modal"  id="subprojectsassigned" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" ></h5>
                        ...
                        <form method="POST" enctype="multipart/form-data"  action="{{url('admin/subprojectssassigned')}}">
                            <input type="hidden" name="projected" id="projects_id">
                            @csrf
                            @honeypot
                            <div class="modal-body">
                                <div class="form-group">
                                	<input type="hidden" name="projected[]" id="projected">
                                    <label class="form-label"> {{trans('langconvert.admindashboard.selectsubproject')}}</label>
                                    <div class="custom-controls-stacked d-md-flex" >
                                        <select multiple="multiple" class="form-control select2_modal_sub"  name="subproject_id[]" data-placeholder="Select Category" id="category" >
                                            @foreach ($subprojects as $category)

											<option value="{{$category->id}}" {{in_array($category->id,$check_subprojects) ? 'selected':''}}>{{$category->name}}</option>
			...
			(function($)  {
				...
				$('#support-articlelists').DataTable({
					...
					columns: [
						...
						{ data: 'name', name: 'name' },
						{ data: 'subprojects', name: 'subprojects' },
				...
				//projects assign sub
				$('#projectsassigns').on('click', function () {
					var id = [];
					...
					if(id.length > 0){ 	
						if(id.length > 1){
							toastr.error('select 1 project');
							return
						}else{ 
							$('.select2_modal_sub').html(""); 
							$('#projected').val(id[0]);
							document.getElementById('projectdisable').style.pointerEvents = 'none';
							document.getElementById('projectdisable').style.opacity = '0.6';
							$.get('projects/'+id[0] , function (data) {
								$('.select2_modal_sub').html(data.subprojectlist);
								$('.select2_modal_sub').select2({
									dropdownParent: ".subproject_modal",
									minimumResultsForSearch: '',
									width: '100%',
								}); 
							});
							...
							$('#projects_form').trigger("reset");
							$('.modal-title').html("{{trans('langconvert.admindashboard.assignprojects')}}");
							$('#subprojectsassigned').modal('show');
							$('#projects').hide();
				...
				$('.pillprojectsassigns').on('click', function () {
					var id = [];
