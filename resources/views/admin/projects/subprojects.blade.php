...
<div class="col-xl-12 col-lg-12 col-md-12">
	<div class="card ">
		<div class="card-header border-0 d-md-max-block">
			<h4 class="card-title mb-md-max-2">Sub Projects</h4>
			<div class="card-options d-md-max-block">
				@can('Project Create')

				<a href="javascript:void(0)" class="btn btn-success me-3 mb-md-max-2 mw-10" id="create-new-projects">{{trans('langconvert.admindashboard.addsubprojects')}}</a>
				@endcan
				<!-- @can('Project Assign')
			...
			<form method="POST" enctype="multipart/form-data" action="{{url('admin/projectsassigned')}}">
				...
				<div class="modal-body">
					<div class="form-group">
						...
						<!-- {{trans('langconvert.admindashboard.selectcategory')}} -->
							Sub Project 
						 </label>
...
<script type="text/javascript">
	...
	(function($) {
		...
		$('body').on('click', '.edit-testimonial', function() {
			...
			$.get('projects/subprojects/' + projects_id, function(data) {
				$('#nameError').html('');
			...
			swal({
				...
				.then((willDelete) => {
					if (willDelete) {
						$.ajax({
							type: "get",
							url: SITEURL + "/admin/projects/subprojects/delete/" + _id,
			...
			if (id.length > 0) {
				swal({
					...
					.then((willDelete) => {
						if (willDelete) {
							$.ajax({
								url: "{{ url('admin/subprojects/massproject/delete')}}",
								method: "GET",
								data: {
									id: id
								},
		...
		$('body').on('submit', '#projects_form', function(e) {
			...
			$.ajax({
				type: 'POST',
				url: SITEURL + "/admin/projects/subprojects/create",
				data: formData,
				cache: false,
				contentType: false,
				processData: false,
				...
				error: function(data) {
					$('#nameError').html('');
					toastr.error('Subproject Name is Already Exists');
					$('#btnsave').html('Save Changes');
				}
			});
